const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4800';
...
const adminCookie = await login('admin@robotics-automation-operations.local', 'admin123');
const managerCookie = await login('manager@robotics-automation-operations.local', 'manager123');
const analystCookie = await login('analyst@robotics-automation-operations.local', 'analyst123');
